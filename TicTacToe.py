    ...
    def _generate_win_conditions(self):
        ...
        for row in range(cols):
            for start_col in range(rows - win_length + 1):
                self._win_conditions.append([start_col + row * rows + offset for offset in range(win_length)])

        for col in range(rows):
            for start_row in range(cols - win_length + 1):
                self._win_conditions.append([col + (start_row + offset) * rows for offset in range(win_length)])
